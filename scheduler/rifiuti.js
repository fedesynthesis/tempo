...
const SEND_HOUR = 21;
const SEND_MIN  = 0;                // 21:00 ora italiana
...
async function run(db, fcm){
  ...
  for(const t of docs){
    if(t.testAt && !(t.testSentAt>=t.testAt)){
      await send(db, fcm, [t.id], '♻️ Rifiuti — prova', 'Le notifiche funzionano. Ti avviso ogni sera alle 21:00 quando c’è da esporre qualcosa.');
